__all__ = ['split_stratified','stratified_subsets','stratified_subset','stratified_batched','stratify']
import numpy as np
import pandas as pd
import os
...
def stratified_subset(df, ratio=0.125, col_label="label_diag_superclass", col_age="age", col_sex="sex", value_male=0, col_patient="patient_id", drop_multiple_samples_per_patient=True):
    ...
    if drop_multiple_samples_per_patient is True:
        df = df.sample(frac=1, random_state=42).drop_duplicates(subset=col_patient, keep='first')
    ...
    df["demographics"]=df.apply(lambda x: (["male"] if x[col_sex]==value_male else ["female"])+[convert_age(x[col_age])],axis=1)
    df["label_strat"]=df.apply(lambda x:x["demographics"]+(x[col_label]), axis=1)
    classes = np.unique([x for xs in df["label_strat"] for x in xs])
    res = stratify(list(df_train.label_strat),classes,[ratio,1-ratio])
    df_res = df.iloc[res[0]].copy().reset_index()
    df_res = df_res.drop(["demographics","label_strat"],axis=1)
    return df_res
#######################################################################################
#low-level routines
#######################################################################################
